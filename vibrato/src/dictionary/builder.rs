...
impl SystemDictionaryBuilder {
    ...
    ///
    /// This function generates a raw connector, which holds the bi-gram costs without converting
    /// them into a connection matrix. Analysis using this dictionary is slower than the model
    /// generated by [`from_readers()`], but the dictionary becomes small and reduces memory
    /// consumption.
    ///
    /// # Arguments
    ///
    ...
    ///  - `char_prop_rdr`: A reader of character definition file `char.def`.
    ///  - `unk_handler`: A reader of unknown definition file `unk.def`.
    ///  - `dual_connector`: If `true`, the model uses a raw connector and a matrix connector
    ///                      together to control speed reduction.
    ///
    /// # Errors
    ///
    /// [`VibratoError`] is returned when an input format is invalid.
